...
public class PdfService {
  ...
  public String getCustcdBySpdate(String fileKey) {
    try {
      log.info("🔹 고객 코드(custcd) 조회 시작: fileKey={}", fileKey);
      String sql = """
                SELECT custcd 
                FROM tb_xa012 
      ...
      log.info("✅ 고객 코드 조회 성공: fileKey={}, custcd={}", fileKey, custcd);
      return custcd;
    } catch (EmptyResultDataAccessException e) {
      log.warn("❌ 고객 코드 없음: fileKey={}", fileKey);
      return null;
    } catch (Exception e) {
      log.error("🚨 고객 코드 조회 중 오류 발생: fileKey={}, error={}", fileKey, e.getMessage(), e);
      return null;
    }
  }
}